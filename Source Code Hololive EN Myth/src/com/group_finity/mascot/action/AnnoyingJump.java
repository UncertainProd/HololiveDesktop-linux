 ...
 */
public class AnnoyingJump extends ActionBase
{
    private static final Logger log = Logger.getLogger(AnnoyingJump.class.getName());
    ...
    public AnnoyingJump( java.util.ResourceBundle schema, final List<Animation> animations, final VariableMap context )
    {
    ...
    {
        // Check if the property is false and disable the jump command
        if (!isJumpEnabled()) {
            return false;
        }
    ...
    {
        // Check if the property is false and skip the jump logic
        if (!isJumpEnabled()) {
            return;
        }
    ...
    // Method to check if the jump is enabled
