 ...
 */
public class DraggedA extends ActionBase
{
    private static final Logger log = Logger.getLogger( DraggedA.class.getName( ) );
    ...
    public DraggedA( java.util.ResourceBundle schema, final List<Animation> animations, final VariableMap context )
    {
    ...
    {
        // Check if the property is true and enable the DraggedA command
        if (!isDraggedAEnabled()) {
            return false;
        }
    ...
    {
        // Check if the property is true and enable the DraggedA logic
        if (!isDraggedAEnabled()) {
            return;
        }
    ...
    // Method to check if the DraggedA command is enabled
    private boolean isDraggedAEnabled() {
        return Boolean.parseBoolean(Main.getInstance().getProperties().getProperty("JumpEnabled", "true"));
    }
}